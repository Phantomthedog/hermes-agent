...
class ContextEngine(ABC):
    ...
    def shutdown(self) -> None:
        """Release per-clone resources (DB handles, file descriptors, etc.).

        Called by AIAgent.close() only when the agent owns a cloned engine
        (i.e. clone_for_agent() returned a distinct object). The default
        no-op is safe for stateless engines and the shared registered
        singleton.
        """
        pass
